...
class Daemon(object):
    ...
    """
    def __init__(self, pidsdir="", stdin='/dev/null',
    ...
    def daemonize(self):
        ...
        # write pidfile
        pid = str(os.getpid())
        file(self.pidsdir+'/gfdaemon.pid','w+').write("%s\n" % pid)
...
----------\n" % time.strftime("%a, %d %b %Y %H:%M:%S +0000", time.gmtime()))
    ...
    running?\n"
            sys.stderr.write(message % (self.pidsdir+'/gfdaemon.pid'))
            sys.exit(1)
    ...
    running?\n"
        ...
        if pid != os.getpid():
            ...
            while True:
                if not os.path.exists(self.pidsdir+'/gfdaemon.pid'):    
                    break
                time.sleep(0.3)
